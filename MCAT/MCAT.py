...
from information import get_infomatrion
...
def COMPOUND_MCAT(smiles_list,i):

    ##32 hou mian xu yao  yon bian liang i ti dai
    model=MultiHeadCrossAttention(num_heads,query_dim,key_dim,value_dim,i).cuda()
    print(get_infomatrion(smiles_list).size())
    output,attention=model(GNN_feature(smiles_list,i),GNN_feature(smiles_list,i),get_infomatrion(smiles_list).cuda(),i)
    return output



#print(get_infomatrion(smiles_list).shape)
